"""
Generate combined ROC and PR curves showing all 3 models in single plots.
Uses actual curve data from retrained models.

NOTE: To regenerate actual curve data, run: python retrain_and_extract_curves.py
This script will use the saved actual_curve_data.json file.
"""
...
def load_curve_data():
    ...
    if not data_path.exists():
        raise FileNotFoundError(
            f"Curve data file not found: {data_path}\n"
            "Please run: python retrain_and_extract_curves.py"
        )
...
def plot_combined_roc_curves():
    ...
    for model_key, model_info in MODELS.items():
        print(f"Processing {model_info['label']}...")
