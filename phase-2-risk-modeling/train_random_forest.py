...
2. Hyperparameter Search: Manual grid search with K-fold CV to find best parameters
   ...
   - Each fold evaluated on holdout set
...
- Progress tracking with manual grid search
...
Requirements:
    pip install scikit-learn pandas joblib matplotlib seaborn datasets
...
def train_model(args: argparse.Namespace):
    ...
    3. Evaluate on fold's holdout
    4. Final evaluation on untouched final_test_set
    """
    ...
    print(f"Running manual grid search with {args.n_splits}-fold cross-validation on development set...")
    ...
    # STEP 3: K-Fold Cross-Validation with Best Parameters
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(cv_kfold.split(X_development, y_development), 1):
        ...
        if hasattr(fold_model, 'oob_score_'):
            oob_score = float(fold_model.oob_score_)
    ...
    save_visualizations(
        ...
        model=final_model, X=X_final_test, feature_names=feature_names
    )
