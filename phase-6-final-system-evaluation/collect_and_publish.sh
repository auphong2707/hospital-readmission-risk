...
################################################################################
# Phase 6 - Final Step: Results Collection & Publication
#
# Collects outputs from all phases (1-6) and publishes to HuggingFace for reproducibility
#
# ARCHITECTURE (Dec 2025):
# ========================
# This is the final step of Phase 6, executed after final_evaluation scripts.
# 
# Phase 1: Collects from data/processed/ (splits and metadata)
# Phase 2: Collects from outputs/${METHOD}/ (models, metrics, visualizations)
# Phase 3: Collects from outputs/${METHOD}/calibration/ (calibrated models, metrics)
# Phase 4: Collects from outputs/${METHOD}/threshold_optimization/ (thresholds, ROI)
# Phase 5: Collects from phase-5-fairness-assessment-mitigation/outputs/${METHOD}/
# Phase 6: Collects from outputs/${METHOD}/final_evaluation/ (AUTHORITATIVE METRICS)
#
# Key Points:
# - Phase 6 final_system_metrics.json is the SINGLE SOURCE OF TRUTH
# - This collection step runs AFTER Phase 6 evaluation completes
# - Aggregates all outputs from the complete pipeline for publication
#
# Usage:
#   bash collect_and_publish.sh --method <method_name> [--repo-id <username/repo>] [OPTIONS]
#
# Methods:
#   - gradient_boosting: Full pipeline with final collection
#   - random_forest: Full pipeline with final collection
#   - logistic_regression: Full pipeline with final collection
################################################################################
...
PROJECT_ROOT="."
OUTPUT_DIR="./outputs"
COLLECTION_DIR="${OUTPUT_DIR}/phase6_final_collection_${METHOD}"
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║      PHASE 6 - FINAL STEP: RESULTS COLLECTION & PUBLICATION                ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo "Phase 6 - Final Collection Summary" > "${SUMMARY_FILE}"
...
aggregated = {
    ...
    "pipeline_summary": {
        ...
        "phase_6": "Final System Evaluation & Results Collection (AUTHORITATIVE)"
    }
}
...
aggregated["collection_metadata"] = {
    ...
    "key_outputs": {
        ...
        "phase_6": ["final_system_metrics.json (AUTHORITATIVE)", "deployment_report.json", "visualizations", "aggregated_results.json", "MODEL_CARD.md"]
    },
    "notes": [
        "This collection is the final step of Phase 6, executed after evaluation",
...
**Method**: ${METHOD}  
**Version**: 1.0 (Complete Pipeline with Final Collection)  
**Date**: $(date +%Y-%m-%d)  
**Repository**: [hospital-readmission-risk](https://github.com/auphong2707/hospital-readmission-risk)

## Model Description

This is a comprehensive machine learning system for predicting 30-day hospital readmission risk for diabetes patients. The system has undergone a complete 6-phase development, evaluation, and collection pipeline:
...
6. **Phase 6**: Final comprehensive system evaluation + results collection (**AUTHORITATIVE METRICS**)
...
5. **Phase 6**: Final evaluation with deployed configuration + comprehensive results collection
...
- **v1.0** (2024): Initial release with complete 6-phase pipeline
  - Phase 6 includes final evaluation + comprehensive results collection
  - Final evaluation produces authoritative metrics
...
### Phase 6: Final System Evaluation & Collection ⭐
...
### Aggregated Results (Phase 6 Final Step)
...
echo ""
echo -e "${GREEN}✅ Phase 6 final collection complete!${NC}"
