"""
Phase 3: Calibrate Random Forest Model for Hospital Readmission Prediction

This script performs comprehensive calibration of the trained Random Forest model from Phase 2.
...
def save_calibrated_model(model, calibrator, output_dir, method):
    ...
    model_path = output_path / "random_forest_model_original.joblib"
    joblib.dump(model, model_path)
    print(f"✅ Original model saved: {model_path}")
    
    # Calibrator is already saved by calibrate_model_pipeline
    calibrator_path = output_path / "Random_Forest_calibrator.pkl"
...
## Overview
This directory contains a calibrated Random Forest model for hospital readmission prediction.
The model has been calibrated using {method.upper()} to ensure reliable probability estimates.

## Files
- `random_forest_model_original.joblib`: Original trained Random Forest model
- `Random_Forest_calibrator.pkl`: Calibration transformer ({method})
- `Random_Forest_report.txt`: Detailed calibration report
- `Random_Forest_metrics.json`: Calibration metrics (JSON)
...
# Load original model
model = joblib.load('random_forest_model_original.joblib')

# Load calibrator
calibrator = ModelCalibrator.load('Random_Forest_calibrator.pkl')
...
See `Random_Forest_report.txt` for detailed calibration metrics.
...
3. **Bootstrap and OOB**: Random Forest uses bootstrap sampling and out-of-bag
   evaluation, but calibration is still recommended for reliable probabilities

4. **Regular Recalibration**: Model should be recalibrated periodically as
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calibrate Random Forest model for hospital readmission prediction",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Default: Platt Scaling calibration (downloads from HuggingFace)
    python calibrate_random_forest.py
    
    # Custom output directory
    python calibrate_random_forest.py --output-dir ./my_calibration_results
    
    # Force re-download from HuggingFace
    python calibrate_random_forest.py --force-download
        """
    )
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default='./calibration_outputs/random_forest',
        help='Directory to save calibration outputs (default: ./calibration_outputs/random_forest)'
    )
    ...
    # Print header
    print_section("🎯 Phase 3: Model Calibration - Random Forest", "=")
    ...
    try:
        ...
        calibrator = ModelCalibrator.load(
            str(Path(args.output_dir) / "Random_Forest_calibrator.pkl")
        )
        ...
        print(f"\n🚀 Next Steps:")
        print(f"   1. Review calibration report: {args.output_dir}/Random_Forest_report.txt")
