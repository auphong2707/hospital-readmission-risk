...
1. Download pre-trained model from HuggingFace Hub (or load from local)
...
HuggingFace Model:
    Repository: https://huggingface.co/auphong2707/hospital-readmission-lr
...
def load_trained_model(model_dir: str = "./models"):
    """
    Load trained Logistic Regression model and scaler from Phase 2.
    ...
    Returns:
        tuple: (model, scaler)
    """
    print_section("📥 Loading Trained Logistic Regression Model", "-")
    
    model_path = Path(model_dir) / "logistic_regression_model.joblib"
    scaler_path = Path(model_dir) / "logistic_regression_scaler.joblib"
    
    if not model_path.exists():
        raise FileNotFoundError(
            f"Model file not found: {model_path}\n"
            f"Please run Phase 2 training first:\n"
            f"  python ./phase-2-risk-modeling/train_logistic_regression.py"
        )
...
def save_calibrated_model(model, scaler, calibrator, output_dir, method):
    ...
    This saves both the original model, scaler, and the calibrator so they can be
    ...
    model_path = output_path / "logistic_regression_model_original.joblib"
    joblib.dump(model, model_path)
    print(f"✅ Original model saved: {model_path}")
    
    # Save scaler
    scaler_path = output_path / "logistic_regression_scaler.joblib"
...
- `logistic_regression_scaler.joblib`: Feature scaler (StandardScaler)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    # Default: Platt Scaling calibration (loads from local models/)
    ...
    python calibrate_logistic_regression.py --model-dir ../models
        """
    )
    ...
    try:
        # STEP 1: Load trained model from Phase 2
        print_section("📥 Step 1: Load Trained Model from Phase 2", "=")
        model, scaler = load_trained_model(model_dir=args.model_dir)
